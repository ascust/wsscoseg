...
def create_main(data, class_num, workspace=512, use_global_stats=True, lr_type="mxnet"):
    ...
    new_conv1 = syms.conv(res5c, name="new_conv1", num_filter=512, pad=12, kernel=3, dilate=12,
                          workspace=workspace, lr_type=lr_type)
    new_relu1 = syms.relu(new_conv1)
    dp1 = syms.dropout(new_relu1)
    fc = syms.conv(dp1, name="fc", num_filter=512, workspace=workspace, lr_type=lr_type)
    fc_relu = syms.relu(fc)
    net = syms.conv(fc_relu, name="score", num_filter=class_num, workspace=workspace, lr_type=lr_type)
    return net
...
def create_infer(class_num, workspace=512):
    data = mx.symbol.Variable(name="data")
    orig_data = mx.symbol.Variable(name="orig_data")
    score = create_main(data, class_num, workspace=workspace)
    softmax = mx.symbol.softmax(score, axis=1)

    up1 = syms.upsample(softmax, name="score_up1", scale=2, num_filter=class_num, workspace=workspace)
    up2 = syms.upsample(up1, name="score_up2", scale=2, num_filter=class_num, workspace=workspace)
    up3 = syms.upsample(up2, name="score_up3", scale=2, num_filter=class_num, workspace=workspace)

    cropped_data = mx.symbol.Crop(up3, orig_data)
