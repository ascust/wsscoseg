...
def create_g_part(data, class_num, workspace=512, lr_type="alex", use_global_stats=True):
    ...
    fc1 = syms.conv(data=pool, kernel=1, num_filter=1024, workspace=workspace, name="g_multi_fc1")
    fc1_relu = syms.relu(fc1)
    dp1 = syms.dropout(data=fc1_relu)
    fc2 = syms.conv(data=dp1, kernel=1, num_filter=1024, workspace=workspace, name="g_multi_fc2")
    fc2_relu = syms.relu(fc2)
    dp2 = syms.dropout(data=fc2_relu)
    score = syms.conv(data=dp2, kernel=1, num_filter=class_num, workspace=workspace, name="g_score")
    return score
...
def create_seg_part(data, class_num, workspace=512, lr_type="alex", use_global_stats=True):
    ...
    net = syms.conv(fc_relu, name="score", num_filter=class_num, workspace=workspace)
    return net
