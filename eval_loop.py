...
def get_untested_list(snapshot_folder, output_folder, model_name):
    untested_list = []
    snapshot_files = os.listdir(conf.SNAPSHOT_FOLDER)
    for f in snapshot_files:
        if f.endswith(".params") and model_name in f:
            ...
            if os.path.exists(os.path.join(conf.OUTPUT_FOLDER, output_folder)):
                continue
...
if __name__ == "__main__":
    ...
    if args.model == "init":
        ...
        model_name = "final_fcn_%s" % conf.BASE_NET

    if args.epoch == 0:
        while True:
            untested_list = get_untested_list(conf.SNAPSHOT_FOLDER, conf.OUTPUT_FOLDER, model_name=model_name)
            ...
            if len(untested_list)>0:
                ...
                print "Waiting for %f hours" % conf.EVAL_WAIT_TIME
                time.sleep(conf.EVAL_WAIT_TIME*3600)
                untested_list = get_untested_list(conf.SNAPSHOT_FOLDER, conf.OUTPUT_FOLDER, model_name=model_name)
                if len(untested_list)>0:
                    continue
                else:
